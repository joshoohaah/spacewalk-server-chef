    ...
    def tgz(self):
        ...
        if self.__tgz is None:
            ...
            if extension in PART_EXTS:
                try:
                    # explicitly set the compressor to be used so that a
                    # CompressionError is raised rather than a ReadError which
                    # you would get if you used mode='r' and let tarfile
                    # determine the compression type on its own.
                    self.__tgz = tarfile.open(fileobj=self.__member,
                                              mode='r:%s'%extension)
                except tarfile.CompressionError as e:
                    # python < 3.3 doesn't have lzma with filehandle support and
                    # tarfile cannot handle xz compressed archives.
                    import subprocess
                    import io
                    ...
                    self.__tgz = tarfile.open(fileobj=fh2, mode='r:')
            else:
